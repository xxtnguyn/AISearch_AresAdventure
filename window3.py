...
def load_grid_from_file(filename):
    ...
    with open(filename, 'r') as file:
        ...
        weights_list = list(map(int, first_line.split()))  # Convert split parts to integers
...
def Window3(level):
    ...
    while running:
        ...
        if len(path) > 0 and is_moving:
            ...
            if direction in ['r', 'R']:
                if direction == 'R':
                    ...
                    # Di chuyển trọng số theo hộp
                    weight_pos[(next_box[0], next_box[1])] = weight_pos[(current_box[0], current_box[1])]
                    weight_pos.pop((current_box[0], current_box[1]), None)
        ...
        for pos, weigh_box in weight_pos.items():
            weight_font_size = 70
            weight_font = pygame.font.Font(font_path, weight_font_size)

            rect = pygame.Rect(pos[1] * TILE_SIZE, pos[0] * TILE_SIZE, TILE_SIZE, TILE_SIZE)
            text = weight_font.render(f"{weigh_box}", True, (255, 0, 0))  # Đoạn text chứa vị trí của TILE
